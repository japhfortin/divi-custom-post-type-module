...
class ET_Builder_Module_Custom_Post_Type extends ET_Builder_Module {
	function init() {
		$this->name       = esc_html__( 'Custom Post Type', 'et_builder' );
		$this->slug       = 'et_pb_custom_post_type';
	...
	function get_fields() {
		...
		foreach ($post_types as $i) {
			$obj = get_post_type_object( $i );
			$options_array[$i] = esc_html__( $obj->labels->singular_name, 'et_builder' );
		}
		...
		$fields = array(
			'custom_post_type' => array(
				'label'             => esc_html__( 'Custom Post Type', 'et_builder' ),
				'type'              => 'select',
				'option_category'   => 'configuration',
				'options'			=> $options_array,
				// add custom post types as options
				/*'options'           => array(
					'winner_exercise' => esc_html__( 'Exercises', 'et_builder' ),
					'winner_trainer' => esc_html__( 'Trainers', 'et_builder' ),
					'winner_class' => esc_html__( 'Classes', 'et_builder' ),
					'winner_nutritiontip' => esc_html__( 'Nutrition Tips', 'et_builder' ),
				),*/
				'description'        => esc_html__( 'Choose custom post type to display.', 'et_builder' ),
			...
			'columns' => array(
				...
				'option_category'   => 'configuration',
				'options'           => array(
					'one' => esc_html__( '1', 'et_builder' ),
					'one_half' => esc_html__( '2', 'et_builder' ),
					'one_third' => esc_html__( '3', 'et_builder' ),
					'one_fourth' => esc_html__( '4', 'et_builder' ),
				),
			...
			'__posts' => array(
				'type' => 'computed',
				'computed_callback' => array( 'ET_Builder_Module_Custom_Post_Type', 'get_blog_posts' ),
	...
	static function get_blog_posts( $args = array(), $conditional_tags = array(), $current_page = array() ) {
		...
		// custom code for getting post type
/*		$query_args['post_type'] = $args['custom_post_type'];

		var_dump($args['custom_post_type']);
*/
		// Get query
	...
	function shortcode_callback( $atts, $content = null, $function_name ) {
		...
		$custom_post_type           = $this->shortcode_atts['custom_post_type'];
		...
		// get custom post type value to be included in query
		...
		if ( have_posts() ) {
			while ( have_posts() ) {
				...
				<?php 
					if ($ctr == $col) { //add et_column_last to last column of the row
						$ctr = 1;
						$columns = $columns . ' et_column_last ';
					} else {
						if ($ctr == 1) {
							$ctr++;
							$columns = $temp_column . ' et_column_first ';
						} else {
							$ctr++;
							$columns = $temp_column;
						}
			...
			<article id="post-<?php the_ID(); ?>" <?php post_class( 'et_pb_post clearfix' . $no_thumb_class . $overlay_class . ' ' . $columns  ); ?>>
...
new ET_Builder_Module_Custom_Post_Type;